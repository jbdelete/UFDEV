...
switch TSType
    case THORB
        [ret ] = mp.ParticleDataReadBIN("J:/RCCDData/perfdataM","BCollisionDataSet",1,0);
        if(ret ~= 0)
            return;
        end
    case THORX
        [ret ] = mp.ParticleDataReadBIN("J:/RCCDData/perfdataM","XCollisionDataSet",1,0);
        if(ret ~= 0)
            return;
        end
    case THORY
        [ret ] = mp.ParticleDataReadBIN("J:/RCCDData/perfdataM","YCollisionDataSet",1,0);
        if(ret ~= 0)
            return;
        end
    case THORZ
        [ret ] = mp.ParticleDataReadBIN("J:/RCCDData/perfdataM","ZCollisionDataSet",1,0);
        if(ret ~= 0)
            return;
        end
    case THORXYZ
    ...
    otherwise
        fprintf("Unrecognized test type\r\n");
        return;
end
SIDELEN = 2;
