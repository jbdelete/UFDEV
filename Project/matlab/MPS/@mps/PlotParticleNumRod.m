...
%*
%******************************************************************/
function PlotParticleNumRod(obj,P,PrintNum)
% CalcAccel(obj,P)
% P =   Particle number
...
if 0
   ...
   % h = surf(x+obj.p(P).rx,y+obj.p(P).ry,z+obj.p(P).rz,'FaceColor',cllr,'FaceAlpha',0.5,'LineStyle',"none") ;
    h = surf(x+obj.p(P).vecp(1),...
            y+obj.p(P).vecp(2),...
            z+obj.p(P).vecp(3),...
            'FaceColor',cllr,'FaceAlpha',1.0) ;
    if PrintNum == true
        t=text(obj.p(P).rx+0.03, obj.p(P).ry+obj.p(P).radius+0.40,...
             obj.p(P).rz+obj.p(P).radius+0.10,sprintf('%d',obj.p(P).inrnum),'Color','black');
        t.FontSize = 18;
    end %======================== EVAL-DBG =====================
 %vectarrow([obj.p(P).rx,obj.p(P).ry,obj.p(P).rz],[obj.p(P).nrx, obj.p(P).nrx, obj.p(P).nrx]);
end
